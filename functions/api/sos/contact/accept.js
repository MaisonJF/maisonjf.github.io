...
import { acceptTrustedContactInvite,declineTrustedContactInvite } from '../../../_lib/sos-runtime.js';
...
export async function onRequestPost({request,env}){
  return withSosApi(async ()=>{
    ...
    if(decision==='decline'){
      const result=await declineTrustedContactInvite({env,token});
      return jsonSos({ok:true,accepted:false,declined:true,idempotent:result.idempotent});
    }
    if(decision!=='accept')return jsonSos({ok:false,error:'invalid_invite_decision'},400);
    const result=await acceptTrustedContactInvite({env,token});
    return jsonSos({ok:true,accepted:true,declined:false,idempotent:result.idempotent});
  });
}
