...
export async function recordB2BEvent(db,input={}){
  ...
  if(idempotencyKey.length<8||idempotencyKey.length>200||DIRECT_EMAIL.test(idempotencyKey))throw new Error('invalid_b2b_idempotency');
  ...
  for(const key of allowed){
    ...
    if(!B2B_TOKEN.test(value)||DIRECT_EMAIL.test(value))throw new Error('invalid_b2b_metadata');
    metadata[key]=value;
  }
