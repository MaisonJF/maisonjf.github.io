export const MAISON_OFFER_BRAIN={
  ...
  principles:{
    ...
    ladder:'Depois da entrada, oferecer aprofundamento e continuidade apenas quando fazem sentido.',
...
export const MAISON_OFFER_CATALOGUE=[
  ...
  {id:'mentoria-pontual',family:'mentoring',stage:'deeper',format:'service',title:'Mentoria Pontual',description:'Um objetivo concreto, estrutura e um plano de execução sem abrir um percurso longo.',href:'/servicos/#mentoria-pontual',amount:12500,priceLabel:'125 €',axes:['direction','control','self'],territories:['cabeca'],routes:['talk','continuity'],base:4},
  {id:'mentoria',family:'mentoring',stage:'continuity',format:'continuity',title:'Mentoria Individual',description:'Um percurso estruturado para aprender, desenvolver ou construir uma área com objetivos e acompanhamento.',href:'/servicos/#mentoria',amount:30000,priceLabel:'300 €',axes:['direction','control','self'],territories:['cabeca'],routes:['continuity'],base:4},
...
export function recommendMaisonOffers(signal={}){
  ...
  const selected=[];
  for(const stage of MAISON_OFFER_BRAIN.stages){
    const candidate=scored.find(x=>x.offer.stage===stage && !selected.some(s=>s.offer.id===x.offer.id) && diversityOk(x.offer,selected));
  ...
  if(selected.length<MAISON_OFFER_BRAIN.maxOffers){
    for(const candidate of scored){
      ...
      if(!diversityOk(candidate.offer,selected))continue;
...
function publicOffer(entry,index){
  const labels={now:'Agora',deeper:'Quero ir mais fundo',continuity:'Quero continuidade'};
  const o=entry.offer;
  return {
    id:o.id,
    slot:o.stage,
    slotLabel:labels[o.stage]||(['Agora','Aprofundar','Continuar'][index]||'Continuar'),
    title:o.title,
