...
def build_summary(*,overlay_path:Path|None=None)->dict[str,Any]:
    ...
    for ref in sorted(ctx.assets):
        ...
        if snap["asset_type"]!="physical_product" or not snap["public"] or snap["lifecycle_status"]!="active":
            continue
    ...
    return {
        ...
        "summary":{
            "active_public_physical_assets":len(rows),
...
def main()->None:
    parser=argparse.ArgumentParser(description="Field-presence-only summary of private physical operations facts.")
    parser.add_argument("--overlay",type=Path)
    args=parser.parse_args()
    print(json.dumps(build_summary(overlay_path=args.overlay),ensure_ascii=False,indent=2))
