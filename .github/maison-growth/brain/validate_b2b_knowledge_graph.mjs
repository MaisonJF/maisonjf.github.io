...
const graph=JSON.parse(fs.readFileSync(path.join(repoRoot,'.github/maison-growth/brain/maison-knowledge-graph.generated.json'),'utf8'));
const professional=graph.professional;
const surface=professional?.surface;

assert(surface,'professional_surface_missing_from_knowledge_graph');
assert(surface.id===ctx.surface.id,'professional_surface_id_mismatch');
...
assert(professional,'professional_layer_missing_from_knowledge_graph');
assert(professional.serviceAssetRef===MAISON_B2B_BRAIN.serviceAssetRef,'b2b_service_asset_ref_mismatch');

const contractSegments=ctx.segments.map(x=>x.id).sort();
const graphSegments=(professional.segments||[]).map(x=>x.id).sort();
...
const graphNeeds=(professional.needs||[]).map(x=>x.id).sort();
assert(JSON.stringify(contractNeeds)===JSON.stringify(graphNeeds),'professional_need_contract_drift');

assert((professional.offerFamilies||[]).length===(MAISON_B2B_BRAIN.offerFamilies||[]).length,'b2b_offer_family_projection_drift');
assert(JSON.stringify(professional.diagnosticOpportunityMap)===JSON.stringify(MAISON_B2B_BRAIN.diagnosticOpportunityMap),'b2b_diagnostic_map_projection_drift');
assert(professional.leadContract?.canonicalEvent==='b2b.lead','b2b_canonical_lead_contract_missing');
assert(professional.commercialReadiness?.status==='quote_framework_ready_values_require_observation','b2b_commercial_readiness_missing');
assert(JSON.stringify(professional.commercialReadiness?.unknownByDefault)===JSON.stringify(MAISON_B2B_BRAIN.commercialReadiness.unknownByDefault),'b2b_commercial_unknowns_projection_drift');
...
assert(professional.leadContract?.runtimeStatus.includes('central_telemetry_or_commerce'),'b2b_lead_contract_must_not_claim_parallel_runtime');
...
assert((professional.researchCandidates||[]).length===(MAISON_B2B_BRAIN.researchCandidates||[]).length,'b2b_research_candidate_projection_drift');
...
assert(professional.professionalNetwork,'professional_network_missing_from_graph');
assert(professional.professionalNetwork.credentialIssuer==='maison-jf','professional_network_credential_authority_drift');
assert(professional.professionalNetwork.franchiseStatus==='not_yet','professional_network_franchise_promoted_without_validation');
assert(professional.professionalNetwork.safeguards?.sublicensing===false,'professional_network_sublicensing_must_default_false');
assert(professional.professionalNetwork.safeguards?.centralCertification===true,'professional_network_certification_must_remain_central');
...
for(const token of ["b2b_business","b2b_result","emitCanonicalB2BLead","/api/b2b-lead"]){
