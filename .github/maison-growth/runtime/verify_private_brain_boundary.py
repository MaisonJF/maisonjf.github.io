...
def _request(
    ...
    *,
    ...
    headers={
        ...
        "User-Agent":"MAISON-JF-Private-Boundary-Verifier/1.0",
    }
    if bearer:
...
def verify_live(stage:str,values:Mapping[str,str])->dict[str,Any]:
    ...
    unauth=_request(
        base,"/internal/brain/health",
        access_client_id=access_id,access_client_secret=access_secret,
    )
    auth=_request(
        base,"/internal/brain/health",bearer=token,
        access_client_id=access_id,access_client_secret=access_secret,
    )
    inbox=_request(
        base,"/internal/brain/action-inbox?limit=1",bearer=token,
        access_client_id=access_id,access_client_secret=access_secret,
    )
    proposal=review=None
    if stage=="private_brain_read_candidate":
        proposal=_request(
            base,"/internal/proposals/a14",method="POST",bearer=token,
            access_client_id=access_id,access_client_secret=access_secret,
        )
        review=_request(
            base,"/internal/reviews/a12",method="POST",bearer=token,
            access_client_id=access_id,access_client_secret=access_secret,
        )
...
def main()->None:
    ...
    result=verify_live(args.stage,os.environ)
